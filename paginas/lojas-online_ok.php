...
function initMap() {    
    ...
    <?php 
    $sql = 'select * from onde_comprar where id_tipo = 3 order by ordem';
    
    if(isset($url4)) {?>
        map.setZoom(7);
        <?php $sql = 'select * from onde_comprar where id_tipo = 3 and id_estado = '.$url4.' order by ordem';?>
    <?php }
    
    if($_POST){
        $local = strtolower($_POST['local']);
        $id_estado = $url4;
        $sql = "select * from onde_comprar where id_estado = ".$id_estado." and id_tipo = 3 and (LOWER(endereco) LIKE '%".$local."%' ) order by ordem";
    } 
        ...
        <div class="row">
            <div class="col-md-12 col-sm-12">
                ...
                <?php foreach (consulta("select b.id, b.nome from onde_comprar a inner join estados b on a.id_estado = b.id group by b.id") as $estados){ ?>
                    <div class="col-md-3">
                        <a class="link-estados <?php echo ($estados['id'] == @$url4 ? 'link-estados-selected' : ''); ?>" href="<?php echo $_SESSION['url'].$lang.'/conteudo/lojas-online/'.$estados['id'];?>">&raquo; <?php echo $estados['nome'];?></a>
                    </div>
                <?php }?>
            </div>
...
<?php
	if($idEstado == 1 or $idEstado == 2 or $idEstado == 3 or $idEstado == 21 or $idEstado == 22 or $idEstado == 14){
         ...
         if($_POST){
            $sql = "select * from onde_comprar where id_tipo = 3 and endereco LIKE '%".$_POST['buscaC']."%' and id_estado = '$idEstado' order by endereco ASC";   
         }
         else{
            $sql = "select * from onde_comprar where id_tipo = 3 and id_estado = '$idEstado' order by ordem";
         }
