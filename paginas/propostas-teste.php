                ...
                success: function (xml) {
                    $(xml).find('transportadoras').each(function () {
                        $(xml).find('transportadora').each(function () {
                            ...
                            $('#cod_transportadora').val($(this).find('pkCodTransp').text());
                        });
                    });           
                },
                error: function () {
    ...
    if ($_POST) {
        ...
        for ($x = 0; $x <= $q; $x++) {
            if ($_POST['codigo_pedido'][$x] != "") {
                if(empty($_POST['descricao_pedido'][$x])){
                    if(empty($_POST['quantidade_pedido'][$x])){
                        if(empty($_POST['unitarios_pedido_format'][$x])){
                            if(empty($_POST['subtotal_pedido_format'][$x])){
                                $erro=1;                                
                                }
                                $erro=1;                                
                            }
                ...
                $m .= "<tr>
                        ...
                        //
                        $txt_maq .=  "\r\n".$_POST['codigo_pedido'][$x]."|".$_POST['quantidade_pedido'][$x]."|".$_POST['unitarios_pedido_format'][$x]."|".$_POST['ipi_pedido'][$x]."|";
            }
        }
        ...
        //CNPJ REP
        $txt = $_POST["cod_cli"]."|".$_POST["cod_transportadora"]."|".$_POST["prazo_pagamento"]."|".$desconto."|".$observacao."|".$txt_maq;
        //Criando um log da hora que o arquivo txt foi criado

        //Retirando tags html para gerar arquivo txt limpo
        /*$txt = str_replace('<table width="100%" border="0" cellspacing="0" cellpadding="0">', " ", $txt);
        $txt = str_replace('<tr>', " ", $txt);
        $txt = str_replace('<td>', " ", $txt);
        $txt = str_replace("<td width='100%'>", " ", $txt);
        ...
        $txt = str_replace('<table width="100%" border="0" cellspacing="10" cellpadding="0">', " ", $txt);
        $txt = str_replace("<td style='text-align: center;'>", " ", $txt);
        $txt = str_replace("<th width='10%'>CODIGO</th>", " ", $txt);
        $txt = str_replace("<th width='10%'>QTD</th>", " ", $txt);
        $txt = str_replace("<th width='15%'>UNITARIOS</th>", " ", $txt);
        $txt = str_replace("<th width='15%'>SUBTOTAL</th>", " ", $txt);
        $txt = str_replace("<th width='10%'>IPI</th>", " ", $txt);
        $txt = str_replace("<th width='25%'>DESCRICAO</th>", " ", $txt);
        $txt = str_replace("<th width='15%'>VALOR IPI</th>", " ", $txt);
        $txt = str_replace('</th>', " ", $txt);
        $txt = str_replace('<table width="100%" border="0" cellspacing="0" cellpadding="0">', " ", $txt);
        $txt = str_replace("<td rowspan='2'>", " ", $txt);*/
        // Criando um novo arquivo
        file_put_contents("/home/croydon/public_html/propostas/".$data.".txt", $txt);
        //Fim
        ...
        else{echo '<script>alert("Erro no processamento! Por favor tente novamente!");</script>';}
    }
}
/*
//Apagar arquivos txt gerados em até 2 dias anterior a data atual.
foreach (glob('/home/croydon/public_html/propostas/*.txt') as $filename) {
if (file_exists($filename)) {
    $diaArquivoModificado = date ("d", filemtime($filename));
    $dataComparaModif =date ("d");
    $dataComparaModif = $dataComparaModif - 1;
    if($dataComparaModif <= $diaArquivoModificado){
       ( unlink( $filename ) ); 
    }
...
<?php if (isset($_SESSION['logado_propostas'])) : ?>
    <form id="form_pedido" action="" name="pedido" method="post">
        ...
        <?php if(isset($_POST["obs"])){ echo $_POST["obs"];} ?>
            <h3>Representante</h3>
        ...
        <div style="margin:0; width:100%; float:left;">
            ...
            <div style="float:left; width:260px; padding-top:120px;display: none;">
                <label>Codigo da Transportadora</label>
                <input class="input_form1" type="text" id="cod_transportadora" name="cod_transportadora" />
            </div>
        </div>
