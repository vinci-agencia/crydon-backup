...
<div class="breadcrumb_internas">
	<div id="breadcrumb">
        ...
        <img src="<?php echo $_SESSION['url'];?>images/seta_bread.gif" alt="icone breadcrumb" />
        <span><?php echo $name_pag;?> Fornos Combinados</span>
    </div>
</div>
<?php 
if(isset($url4)){ ?>
...
<ul class="estados">
	<?php
	 foreach (consulta("select nome,id from estados ORDER BY nome ASC") as $estados){
		 ...
		 if($quant > 0){
	?>
	<li>&raquo; <a class="<?php echo $classe;?>" href="<?php echo $_SESSION['url'].$lang?>/conteudo/autorizada/<?php echo $id_estado;?>"><?php echo $nome_estado;?></a></li>
    <?php
		}
	 }
	 ?>
...
<?php
	 if($idEstado == 1 or $idEstado == 2 or $idEstado == 3 or $idEstado == 21 or $idEstado == 22 or $idEstado == 14){
         ...
         if($_POST){
            $sql = "select * from autorizadas where endereco LIKE '%".$_POST['buscaC']."%' and id_estado = '$idEstado' and tipo= 2 order by endereco ASC";   
         }
         else{
            $sql = "select * from autorizadas where id_estado = '$idEstado' and tipo= 2 order by ordem";
         }
