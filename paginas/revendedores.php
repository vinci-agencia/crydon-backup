...
<div class="container" id="revendedores">
    ...
    <div class="conteudo-pagina">
        <div class="row"><p>Conhe&ccedil;a nossos revendedores no Brasil</p></div>
...
if(isset($_POST['busca'])){
	...
	$busca = $_POST['busca'];	
	$sql = "SELECT nome,id_estado,cidades,endereco,id,ordem,telefone FROM onde_comprar WHERE nome LIKE '%".$busca."%' AND id_estado = ".$idestado." ORDER BY ordem ASC LIMIT $inicial, $numreg";
	$sql_conta = mysql_query("SELECT id FROM onde_comprar WHERE nome LIKE '%".$busca."%' AND id_estado = ".$idestado."");
}
	...
	$id_est = @$_GET['id_est'];
        $sql = "select nome,id_estado,cidades,endereco,id,ordem,telefone from onde_comprar where endereco LIKE '%".$_POST['buscaC']."%' order by endereco ASC LIMIT $inicial, $numreg";   
	$sql_conta = mysql_query("SELECT id FROM onde_comprar where endereco LIKE '%".$_POST['buscaC']."%'");
}
else{
	$id_est = @$_GET['id_est'];
	$sql = "SELECT nome,id_estado,cidades,endereco,id,ordem,telefone FROM onde_comprar WHERE id_estado = '$id_est' ORDER BY ordem ASC LIMIT $inicial, $numreg";
	$sql_conta = mysql_query("SELECT id FROM onde_comprar WHERE id_estado = '$id_est'");
}
...
if(isset($_GET['ordem'])){
	...
	if($ordem == 'maior'){
		$res1 = mysql_fetch_array(mysql_query("SELECT ordem FROM onde_comprar WHERE id = '$id'"));
		$ordem1 = $res1['ordem'];
		$ordem2 = $ordem1 - 1;
		$res2 = mysql_fetch_array(mysql_query("SELECT id FROM onde_comprar WHERE ordem = '$ordem2' and id_estado = '$est'"));
		$id2 = $res2['id'];
		$sql1 = "UPDATE onde_comprar SET ordem = '$ordem2' WHERE id = '$id' and id_estado = '$est'";
		$sql2 = "UPDATE onde_comprar SET ordem = '$ordem1' WHERE id = '$id2' and id_estado = '$est'";
		mysql_query($sql1);
		mysql_query($sql2);
	}
	else{
        ...
        echo "<script>location.href = '".$_SESSION['url']."admin/onde_comprar.php?id_est=".$_GET['id_est']."';</script>";
    ...
    $res1 = mysql_fetch_array(mysql_query("SELECT ordem FROM onde_comprar WHERE id = '$id' and id_estado = '$est'"));
    $sql1 = "UPDATE onde_comprar SET ordem = '$val' WHERE id = '$id' and id_estado = '$est'";

    $res2 = mysql_fetch_array(mysql_query("SELECT ordem,id FROM onde_comprar WHERE ordem = '".$val."' and id_estado = '".$est."'"));
    $sql2 = "UPDATE onde_comprar SET ordem = '".$res1['ordem']."' WHERE id = '".$res2['id']."' and id_estado = '$est'";
...
<!-- PESQUISA -->	
        ...
        <div class="row">
            <div class="col-md-6 col-sm-6">
                ...
                <?php endif ?>
                <?php foreach (consulta("select b.id, b.nome from onde_comprar a inner join estados b on a.id_estado = b.id group by b.id order by nome") as $estados){ ?>
                    <div class="col-md-6">
                        <a class="btn btn-success link-estados <?php echo ($estados['id'] == @$url4 ? 'link-estados-selected' : ''); ?>" href="<?php echo $_SESSION['url'].$lang.'/conteudo/revendedores/'.$estados['id'];?>"><?php echo $estados['nome'];?></a>
                    </div>
                <?php }?>
            </div>
